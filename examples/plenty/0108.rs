...
fn make_trainer() -> Trainer<AdamW<ExecutionContext>, ChessBucketsMirrored, MaterialCount<8>> {
    ...
    return ValueTrainerBuilder::default()
        ...
        .build(|builder, stm, ntm, buckets| {
            ...
            // Build layers
            let l1 = builder.new_affine("l1", L1_SIZE + 64, OUTPUT_BUCKETS * L2_SIZE);
            ...
            let ones = builder.new_constant(Shape::new(56, 1), &[1.0; 32]);
            let stm_subnet_32 = stm_subnet.reshape(Shape::new(56, 32));
            let stm_sums = stm_subnet_32.matmul(ones);
            let ntm_subnet_32 = ntm_subnet.reshape(Shape::new(56, 32));
            let ntm_sums = ntm_subnet_32.matmul(ones);
            let element_wise_mul = stm_sums * ntm_sums;
            let padding = builder.new_constant(Shape::new(8, 1), &[0.0, 8);

            let out = stm_subnet.concat(ntm_subnet).concat(element_wise_mul).concat(padding);
...
fn main() {
    ...
    train(
        ...
        NetConfig { name: "0107r", superbatch: 300 },
        Some(NetConfig { name: "0107", superbatch: 300 }),
    );

    // Step 3
    train(
        "/mnt/d/Chess Data/Selfgen/20ksn.data", // with 11008
        wdl::ConstantWDL { value: 0.6 },
        lr::CosineDecayLR { initial_lr: 0.00025, final_lr: 0.00025 * 0.3 * 0.3 * 0.3, final_superbatch: 400 },
        NetConfig { name: "0107rr", superbatch: 400 },
        Some(NetConfig { name: "0107r", superbatch: 300 }),
    );
}
