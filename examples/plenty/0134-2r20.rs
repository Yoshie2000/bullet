...
fn make_trainer() -> ValueTrainer<AdamW<CudaDevice>, ThreatInputsBucketsMirrored, MaterialCount<8>> {
    ...
    return ValueTrainerBuilder::default()
        ...
        .loss_fn(|output: bullet_lib::nn::NetworkBuilderNode<'_, bullet_cuda_backend::CudaMarker>, targets| {
            ...
            let is_greater = (1.0 + sign) / 2.0;
            let factor = is_greater * (1.1 + diff / 4.0) + (1.0 - is_greater) * 1.0;
...
fn main() {
    ...
    train(
        ...
        NetConfig { name: "0134-2r20", superbatch: 400 },
