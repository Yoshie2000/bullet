...
pub fn map_piece_threat( // maps a threat to a feature index (still need to add half width in case of nstm)
...
impl inputs::SparseInputType for ThreatInputsBucketsMirrored {
    ...
    fn map_features<F: FnMut(usize, usize)>(&self, pos: &Self::RequiredDataType, mut f: F) {
        ...
        Chess768.map_features(pos, |stm, ntm| f(TOTAL_THREATS + stm_bucket + (stm ^ stm_flip), TOTAL_THREATS + ntm_bucket + (ntm ^ ntm_flip)));
        ...
        let mut stm_count = 0;
        let mut stm_feats = [0; 128];
        map_features_bucketed(bbs, |stm| {stm_feats[stm_count] = stm; stm_count += 1;});
        ...
        let mut ntm_count = 0;
        let mut ntm_feats = [0; 128];
        map_features_bucketed(bbs, |ntm| {ntm_feats[ntm_count] = ntm; ntm_count += 1;});
...
// --------------------------------------------------
// Results of 0091r vs 0091 (20000 nodes, 1t, 16MB, Pohl.epd):
// Elo: -1.39 +/- 6.13, nElo: -2.18 +/- 9.63
// LOS: 32.83 %, DrawRatio: 44.60 %, PairsRatio: 0.99
// Games: 5000, Wins: 1437, Losses: 1457, Draws: 2106, Points: 2490.0 (49.80 %)
// Ptnml(0-2): [114, 581, 1115, 591, 99], WL/DD Ratio: 1.39
// --------------------------------------------------
// STC 0091r vs main: -4x.xx
...
fn main() {
    ...
    let start_epoch = 1;
    let experiment_name = "0094";
    let data_paths: Vec<String> = fs::read_dir("/mnt/d/Chess Data/Selfgen/Data").unwrap().filter_map(Result::ok).filter(|entry| entry.file_type().is_ok_and(|e| e.is_file())).map(|entry| entry.path().display().to_string()).collect();
    ...
    let data_loader: loader::DirectSequentialDataLoader = loader::DirectSequentialDataLoader::new(&paths);
    if start_epoch != 1 {
        trainer.load_from_checkpoint(format!("/mnt/d/Chess Data/Selfgen/Training/{}/net-{}-{}", experiment_name, experiment_name, start_epoch - 1).as_str());
    }
    let schedule = TrainingSchedule {
        ...
        steps: TrainingSteps {
            ...
            end_superbatch: 1000, // 420,
        },
        wdl_scheduler: wdl::ConstantWDL { value: 0.35 },
        lr_scheduler: lr::CosineDecayLR { initial_lr: 0.001, final_lr: 0.001 * 0.3 * 0.3 * 0.3 * 0.3, final_superbatch: 1000 },
        save_rate: 1,
    };
