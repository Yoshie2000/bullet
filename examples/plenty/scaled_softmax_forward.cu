    ...
    float* output)
{
    ...
    {
        this_output[loc_in_elem] = this_elem[loc_in_elem] * this_elem[loc_in_elem];
        // this_output[loc_in_elem] = (sigmoid(k * (this_elem[loc_in_elem] - 0.5F)) - sigmoid(-0.5F * k)) / (sigmoid(0.5F * k) - sigmoid(-0.5F * k));
    }
}