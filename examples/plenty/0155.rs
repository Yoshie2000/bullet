...
fn main() {
    train(
        ...
        wdl::Sequence {
            first: wdl::LinearWDL { start: 0.15, end: 0.6 },
            first_scheduler_final_superbatch: 800,
            second: wdl::Sequence {
                first: wdl::LinearWDL { start: 0.6, end: 1.0 },
                first_scheduler_final_superbatch: 300,
                second: wdl::ConstantWDL { value: 1.0 },
            }
        },
        lr::Sequence {
            first: lr::CosineDecayLR { initial_lr: 0.001, final_lr: 0.001 * 0.3 * 0.3 * 0.3, final_superbatch: 800 },
            first_scheduler_final_superbatch: 800,
            second: lr::Sequence {
                first: lr::LinearDecayLR { initial_lr: 0.00025, final_lr: 0.0000025, final_superbatch: 300 },
                first_scheduler_final_superbatch: 300,
                second: lr::LinearDecayLR { initial_lr: 0.0000025, final_lr: 0.000025 * 0.3 * 0.3 * 0.3, final_superbatch: 300 },
            }
        },
        NetConfig { name: "0155", superbatch: 1400 },
        None,
    );
}
