...
fn make_trainer() -> ValueTrainer<AdamW<CudaDevice>, ThreatInputsBucketsMirrored, MaterialCount<8>> {
    ...
    return ValueTrainerBuilder::default()
        ...
        .build_custom(|builder, (stm, ntm, buckets), targets| {
            ...
            // Extract WDL & score from l3 output
            let out_wdl = l3_out.slice_rows(0, 3);
            let out_score_mask = builder.new_constant(Shape::new(1, 4), &[0.0, 0.0, 0.0, 1.0]);
            let out_score = out_score_mask.matmul(l3_out);

            // Calculate loss for WDL head
            let ones = builder.new_constant(Shape::new(1, 3), &[1.0; 3]);
            let wdl_loss = ones.matmul(out_wdl.softmax_crossentropy_loss(targets));

            // Calculate loss for score head (assuming 1.0 WDL)
            let target_draw_mask = builder.new_constant(Shape::new(1, 3), &[0.0, 1.0, 0.0]);
            let target_win_mask = builder.new_constant(Shape::new(1, 3), &[0.0, 0.0, 1.0]);

            let target_draw = target_draw_mask.matmul(targets);
            let target_win = target_win_mask.matmul(targets);

            let target_score = 0.5 * target_draw + target_win;
            let score_loss = out_score.sigmoid().squared_error(target_score);

            let final_loss = (0.1 * wdl_loss + score_loss) / 2.0;
...
fn main() {
    ...
    train(
        ...
        NetConfig { name: "0161r", superbatch: 400 },
        Some(NetConfig { name: "0161", superbatch: 1000 }),
    );
}
