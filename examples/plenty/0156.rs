...
fn make_trainer() -> ValueTrainer<AdamW<CudaDevice>, ThreatInputsBucketsMirrored, MaterialCount<8>> {
    ...
    return ValueTrainerBuilder::default()
        ...
        .build(|builder, stm, ntm, buckets| {
            ...
            let l3 = builder.new_affine("l3", L3_SIZE + 2 * L2_SIZE, OUTPUT_BUCKETS * L4_SIZE);
            ...
            let l3_out = l3.forward(l2_out.concat(l1_out)).select(buckets).crelu();
            let l4_out = l4.forward(l3_out).select(buckets);
...
fn main() {
    ...
    train(
        ...
        NetConfig { name: "0156", superbatch: 1000 },
        None,
    );
    ...
    train(
        ...
        NetConfig { name: "0156r", superbatch: 400 },
        Some(NetConfig { name: "0156", superbatch: 1000 }),
    );
}
