...
impl inputs::SparseInputType for ThreatInputsBucketsMirrored {
    ...
    fn num_inputs(&self) -> usize {
        768 + TOTAL_THREATS + 768 * self.num_buckets + 1
    }
    ...
    fn map_features<F: FnMut(usize, usize)>(&self, pos: &Self::RequiredDataType, mut f: F) {
        ...
        // Calculate simple eval to sometimes skip threat features
        const PIECE_VALUES: [i32; 7] = [1, 3, 3, 5, 9, 0, 0];
        ...
        if material_diff >= 3 {
            let skip_feature = self.num_inputs() - 1;
            f(skip_feature, skip_feature);
            return;
        }
...
fn make_trainer() -> ValueTrainer<AdamW<CudaDevice>, ThreatInputsBucketsMirrored, MaterialCount<8>> {
    ...
    return ValueTrainerBuilder::default()
        ...
        .build(|builder, stm, ntm, buckets| {
            // Build layers
            let l0 = builder.new_affine("l0", 768 + TOTAL_THREATS + 768 * KING_BUCKETS + 1, L1_SIZE);
...
fn main() {
    ...
    train(
        ...
        NetConfig { name: "0142", superbatch: 1000 },
        None,
    );
    ...
    train(
        ...
        NetConfig { name: "0142r", superbatch: 400 },
        Some(NetConfig { name: "0142", superbatch: 1000 }),
    );
}
