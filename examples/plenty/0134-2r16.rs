...
fn make_trainer() -> ValueTrainer<AdamW<CudaDevice>, ThreatInputsBucketsMirrored, MaterialCount<8>> {
    ...
    return ValueTrainerBuilder::default()
        ...
        .loss_fn(|output, targets| {
            let sgm = output.sigmoid();
            let mse = sgm.squared_error(targets);
            let tipping_point_distance = ((sgm - 0.5).abs_pow(1.0) - 0.375).abs_pow(1.0);
            // https://www.desmos.com/calculator/8prvh1mqmy?lang=de
            let tipping_point_distance = (tipping_point_distance / 0.025).sigmoid();
            let tpe = 0.125 * tipping_point_distance * (1.0 - tipping_point_distance);
            mse + tpe
        })
        .inputs(inputs)
