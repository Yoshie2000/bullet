...
fn make_trainer() -> ValueTrainer<AdamW<CudaDevice>, ThreatInputsBucketsMirrored, MaterialCount<8>> {
    ...
    return ValueTrainerBuilder::default()
        ...
        .build_custom(|builder, (stm, ntm, buckets), targets| {
            ...
            let l3 = builder.new_affine("l3", L3_SIZE + 2 * L2_SIZE, OUTPUT_BUCKETS * 4);
            ...
            let out_loss_mask = builder.new_constant(Shape::new(1, 4), &[1.0, 0.0, 0.0, 0.0]);
            let out_draw_mask = builder.new_constant(Shape::new(1, 4), &[0.0, 1.0, 0.0, 0.0]);
            let out_win_mask = builder.new_constant(Shape::new(1, 4), &[0.0, 0.0, 1.0, 0.0]);
            let out_score_mask = builder.new_constant(Shape::new(1, 4), &[0.0, 0.0, 0.0, 1.0]);

            let out_loss = out_loss_mask.matmul(l3_out);
            let out_draw = out_draw_mask.matmul(l3_out);
            let out_win = out_win_mask.matmul(l3_out);
            let out_score = out_score_mask.matmul(l3_out);

            // Strange way to do a max() operation
            fn max<'a>(x: GraphBuilderNode<'a, CudaMarker>, y: GraphBuilderNode<'a, CudaMarker>) -> GraphBuilderNode<'a, CudaMarker> {
                (x - y).relu() + y
            }
            let out_max = max(out_loss, max(out_draw, out_win));
            ...
            let out_loss = exp(out_loss - out_max);
            let out_draw = exp(out_draw - out_max);
            let out_win = exp(out_win - out_max);

            let inv_sum = (out_win + out_draw + out_loss).abs_pow(-1.0);
            let out_win = out_win * inv_sum;
            let out_draw = out_draw * inv_sum;

            let out_result = (out_draw * 0.5 + out_win).crelu(); // .clamp(0.0, 1.0)
            let wdl_loss = out_result.squared_error(targets);

            let score_loss = out_score.sigmoid().squared_error(targets);

            let final_loss = (wdl_loss + score_loss) / 2.0;

            (out_score, final_loss)
        });
}
...
fn main() {
    ...
    train(
        ...
        NetConfig { name: "0161", superbatch: 1000 },
        None,
    );
}
