...
fn make_trainer() -> ValueTrainer<AdamW<CudaDevice>, ThreatInputsBucketsMirrored, MaterialCount<8>> {
    ...
    return ValueTrainerBuilder::default()
        ...
        .build(|builder, stm, ntm, buckets| {
            ...
            let l2 = builder.new_affine("l2", 2 * L2_SIZE, OUTPUT_BUCKETS * 2 * L3_SIZE);
            ...
            let l2_out = l2_out.slice_rows(0, L3_SIZE) * hardSwish6(l2_out.slice_rows(L3_SIZE, 2 * L3_SIZE));
...
fn main() {
    ...
    train(
        ...
        NetConfig { name: "0168r", superbatch: 400 },
        Some(NetConfig { name: "0168", superbatch: 1000 }),
    );
}
