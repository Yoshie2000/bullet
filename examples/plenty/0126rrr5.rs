...
-> ValueTrainer<AdamW<CudaDevice>, ThreatInputsBucketsMirrored, MaterialCount<8>> {
    ...
    return ValueTrainerBuilder::default()
        ...
        .build(|builder, stm, ntm, buckets| {
            fn quantised_forward<'a>(builder: &'a GraphBuilder<CudaMarker>, layer: Affine<'a, CudaMarker>, input: GraphBuilderNode<'a, CudaMarker>, value: f32) -> GraphBuilderNode<'a, CudaMarker> {
                let quantised_weights = builder.apply(FauxQuantise { input: layer.weights.annotated_node(), value, round: true });
                let quantised_bias = builder.apply(FauxQuantise { input: layer.bias.annotated_node(), value, round: true });
                quantised_weights.matmul(input) + quantised_bias
            }

            const INPUT_QUANT: i32 = 127;
            const INPUT_SHIFT_1: i32 = 5;
            const INPUT_SHIFT_2: i32 = 5;
            const PACKUS_QUANT: i32 = ((INPUT_QUANT << INPUT_SHIFT_1) * (INPUT_QUANT << INPUT_SHIFT_2)) >> 16;
            ...
            // Crelu + Pairwise
            let stm_subnet = quantised_forward(builder, l0, stm, INPUT_QUANT as f32).crelu().pairwise_mul();
            let ntm_subnet = quantised_forward(builder, l0, ntm, INPUT_QUANT as f32).crelu().pairwise_mul();
            let pairwise_out = builder.apply(FauxQuantise { input: stm_subnet.concat(ntm_subnet).annotated_node(), value: PACKUS_QUANT as f32, round: true });
            // Dual activation
            let l1_out = quantised_forward(builder, l1, pairwise_out, L1_QUANT as f32).select(buckets);
...
fn main() {
    ...
    train(
        ...
        NetConfig { name: "0126rrr5", superbatch: 300 },
        Some(NetConfig { name: "0126rr", superbatch: 400 }),
    );
}
