...
fn piece_count_acceptance(board: &Board) -> f64 {
    ...
    static PIECE_COUNT_ALL_STATS: [AtomicU64; 33] = {
    ...
    static PIECE_COUNT_ALL_TOTAL: AtomicU64 = AtomicU64::new(0);
    const MAX_SKIPPING_RATE: f64 = 10.0;

    let pc = board.pieces.occupied().count() as usize;
    let count = PIECE_COUNT_ALL_STATS[pc].fetch_add(1, Ordering::Relaxed) + 1;
    let total = PIECE_COUNT_ALL_TOTAL.fetch_add(1, Ordering::Relaxed) + 1;

    let tmp = (total as f64 * DESIRED_DISTRIBUTION[pc] / count as f64).min(total as f64);
    let alpha = 1.0 / (tmp * MAX_SKIPPING_RATE);

    let tmp = (alpha * tmp).min(1.0); // in nnue-pytorch, positions get filtered out with probability 1 - tmp => acceptance probability == tmp
    tmp
}
