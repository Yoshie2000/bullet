...
fn make_trainer() -> ValueTrainer<AdamW<CudaDevice>, ThreatInputsBucketsMirrored, MaterialCount<8>> {
    ...
    return ValueTrainerBuilder::default()
        ...
        .build(|builder, stm, ntm, buckets| {
            ...
            let l3 = builder.new_affine("l3", L3_SIZE + 2 * L2_SIZE, OUTPUT_BUCKETS * 2);
            ...
            let l3_out_x = l3_out.slice_rows(0, 1);
            let l3_out_y = l3_out.slice_rows(1, 2);
            l3_out_x * (1.0 + l3_out_y.crelu())
        });
}
...
fn main() {
    ...
    train(
        ...
        NetConfig { name: "0163r", superbatch: 400 },
        Some(NetConfig { name: "0163", superbatch: 1000 }),
    );
}
