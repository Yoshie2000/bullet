...
fn make_trainer() -> ValueTrainer<AdamW<CudaDevice>, ThreatInputsBucketsMirrored, MaterialCount<8>> {
    ...
    return ValueTrainerBuilder::default()
        ...
        .build(|builder, stm, ntm, buckets| {
            ...
            let l0_stm = l0.forward(stm);
            let l0_ntm = l0.forward(ntm);
            
            let pstm = builder.apply(ExtractPSTInputs::new(stm));
            let pntm = builder.apply(ExtractPSTInputs::new(ntm));
            let l0_pstm = l0p.forward(pstm);
            let l0_pntm = l0p.forward(pntm);

            let l0_stm = l0_stm.concat(l0_pstm).crelu().pairwise_mul();
            let l0_ntm = l0_ntm.concat(l0_pntm).crelu().pairwise_mul();
            
            // L1 concat + activation
            let l1_out = l1.forward(l0_stm.concat(l0_ntm)).select(buckets);
...
fn main() {
    ...
    train(
        ...
        NetConfig { name: "0149", superbatch: 1000 },
        None,
    );
    ...
    train(
        ...
        NetConfig { name: "0149r", superbatch: 400 },
        Some(NetConfig { name: "0149", superbatch: 1000 }),
    );
}
