...
use bullet_lib::{
    ...
    }, game::inputs::ChessBucketsMirrored, lr::{self, LrScheduler}, nn::{optimiser::{self, AdamWOptimiser}, InitSettings}, trainer::{save::SavedFormat, NetworkTrainer}, value::{ValueTrainer, ValueTrainerBuilder}, wdl::{self, WdlScheduler}, Activation, ExecutionContext, LocalSettings, Shape, TrainingSchedule, TrainingSteps
};
use rand::Rng;
...
fn main() {
    let load_from_net = NetConfig { name: "0107rr3", superbatch: 400 };
    let mut trainer = make_trainer();
    let _ = trainer.load_from_checkpoint(
                format!(
                    "/mnt/d/Chess Data/Selfgen/Training/{}/net-{}-{}",
    ...
    println!("{}", trainer.eval("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1") * 400.0);

    // Step 4
    // train(
    //     "/mnt/d/Chess Data/Selfgen/20ksn-nodeadvantage.data",
    //     wdl::ConstantWDL { value: 1.0 },
    //     lr::CosineDecayLR { initial_lr: 0.000025, final_lr: 0.000025 * 0.3 * 0.3 * 0.3, final_superbatch: 300 },
    //     NetConfig { name: "0107rr3r2", superbatch: 300 },
