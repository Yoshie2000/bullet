...
fn eval_acceptance(eval: i16) -> f64 {
    #[rustfmt::skip]
    const DESIRED_DISTRIBUTION: [f64; 13] = [
        0.143, 0.132, 0.121, 0.11, 0.099, 0.088, 0.077, 0.066, 0.055, 0.044, 0.033, 0.022, 0.011,
    ];

    static EVAL_BUCKET_STATS: [AtomicU64; 13] = {
        let mut arr: [std::mem::MaybeUninit<AtomicU64>; 13] = [const { MaybeUninit::uninit() }; 13];
        let mut i = 0;
        while i < 13 {
            arr[i].write(AtomicU64::new(0));
            i += 1;
        }
        unsafe { std::mem::transmute::<_, [AtomicU64; 13]>(arr) }
    };
    static EVAL_BUCKET_TOTAL: AtomicU64 = AtomicU64::new(0);

    let pc: usize = (eval.abs() / 200).min(12) as usize;
    let count = EVAL_BUCKET_STATS[pc].fetch_add(1, Ordering::Relaxed) + 1;
    let total = EVAL_BUCKET_TOTAL.fetch_add(1, Ordering::Relaxed) + 1;
    let frequency = count as f64 / total as f64;
...
fn filter(board: &Board, mv: Move, eval: i16, wdl: f32) -> bool {
    ...
    !default_viri_filter.should_filter(mv, eval as i32, board, wdl, &mut rng)
        ...
        && rng.random_bool(eval_acceptance(eval))
}
...
fn main() {
    ...
    train(
        ...
        NetConfig { name: "0134-2r29", superbatch: 400 },
