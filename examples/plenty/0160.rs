...
fn make_trainer() -> ValueTrainer<AdamW<CudaDevice>, ThreatInputsBucketsMirrored, MaterialCount<8>> {
    ...
    return ValueTrainerBuilder::default()
        ...
        .build_custom(|builder, (stm, ntm, buckets), targets| {
            ...
            let result = (draw * 0.5 + win).crelu(); // .clamp(0.0, 1.0)
            let loss = result.squared_error(targets);
            (result, loss) // result technically isn't the correct output here, but it's not relevant for training
        });
}
...
fn main() {
    ...
    train(
        ...
        NetConfig { name: "0160", superbatch: 1000 },
        None,
    );
    ...
    train(
        ...
        NetConfig { name: "0160r", superbatch: 400 },
